...
void curved_fluxes(
  ...
  const DataVector conformal_factor_power =
      pow<4>(1 + get(conformal_factor_minus_one));
...
void face_fluxes(
  ...
  const DataVector conformal_factor_power =
      pow<4>(1 + get(conformal_factor_minus_one));
  const DataVector shift_term =
      get(dot_product(shift_rolloff, face_normal)) /
      square(1 + get(lapse_times_conformal_factor_minus_one) /
                     (1 + get(conformal_factor_minus_one)));
...
tnsr::I<DataVector, 3> flux_part_linearization(
  ...
  for (size_t i = 0; i < 3; i++) {
    result.get(i) = -4 * get(conformal_factor_correction) *
                    ...
                    pow<5>(1 + get(conformal_factor_minus_one));
...
void add_GB_terms(
  ...
  const tnsr::ii<DataVector, 3> extrinsic_curvature = Xcts::extrinsic_curvature(
      conformal_factor, lapse, conformal_metric,
      longitudinal_shift_minus_dt_conformal_metric, extrinsic_curvature_trace);
  ...
  get(*scalar_tensor_equation) -=
      2. * (weyl_electric_scalar.get() - weyl_magnetic_scalar.get()) *
...
void Fluxes::apply(
  ...
  ::Xcts::Fluxes<::Xcts::Equations::HamiltonianLapseAndShift,
                 ::Xcts::Geometry::Curved>::
      apply(flux_for_conformal_factor, flux_for_lapse_times_conformal_factor,
...
void LinearizedFluxes::apply(
  ...
  ::Xcts::Fluxes<::Xcts::Equations::HamiltonianLapseAndShift,
                 ::Xcts::Geometry::Curved>::
      apply(flux_for_conformal_factor_correction,
            flux_for_lapse_times_conformal_factor_correction,
            longitudinal_shift_excess_correction, conformal_metric,
            inv_conformal_metric, christoffel_second_kind,
...
void Sources::apply(
  ...
  add_GB_terms(scalar_equation, eps2, eps4,
               lapse_times_conformal_factor_minus_one,
  ...
  ::Xcts::Sources<
      ::Xcts::Equations::HamiltonianLapseAndShift, ::Xcts::Geometry::Curved,
      0>::apply(hamiltonian_constraint, lapse_equation, momentum_constraint,
