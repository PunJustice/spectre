 ...
 * `ScalarizedGr` solutions is probably a good conformal background to solve for
 * a binary solution (see Xcts::AnalyticData::Binary).
 *
...
class ScalarizedGr : public elliptic::analytic_data::Background,
 ...
 public:
  ...
  using options =
      tmpl::flatten<tmpl::list<Amplitude, typename GrSolution::options>>;
  static constexpr Options::String help = GrSolution::help;
  static std::string name() { return pretty_type::name<GrSolution>(); }

  ScalarizedGr() = default;
  ScalarizedGr(const ScalarizedGr&) = default;
  ScalarizedGr& operator=(const ScalarizedGr&) = default;
  ScalarizedGr(ScalarizedGr&&) = default;
  ScalarizedGr& operator=(ScalarizedGr&&) = default;
  ~ScalarizedGr() = default;
